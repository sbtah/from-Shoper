...
def update_or_create_category_stock(
    ...
    try:
        ...
        if (
            ...
            stock.save()
            logging.info(f"!! Stock updated: {stock.shoper_stock_id} Updated")
        else:
            parrent_product = Product.objects.get(
                shoper_id=stock.shoper_stock_product_id
            )
            parrent_product.stock_set.add(stock)
            logging.info(f"No updates for Stock: {stock.shoper_stock_id}")
        ...
        parrent_product.stock_set.add(stock)
        logging.info(f"!! Stock created: {stock.shoper_stock_id} Created")
    return stock
